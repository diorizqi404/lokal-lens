...
const AchievementBanner = ({ certificates = [] }: AchievementBannerProps) => {
  ...
  const handleViewCertificate = () => {
    router.push(`/profil/sertifikat/${latestCertificate.id}`);
  };

  return (
    <section className="w-full bg-white pb-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-center gap-6 p-6 rounded-3xl bg-[#D4A017] shadow-[0_10px_15px_-3px_rgba(0,0,0,0.1),0_4px_6px_-4px_rgba(0,0,0,0.1)]">
          ...
          <div className="flex-1 flex flex-col gap-1">
            ...
            <p className="text-sm font-normal leading-5 text-[rgba(26,26,26,0.9)]">
              {latestCertificate.description || 'Pencapaian luar biasa ini berhak mendapatkan pengakuan. Klaim dan bagikan sertifikat digital Anda.'}
            </p>
          </div>

          <button 
            onClick={handleViewCertificate}
            className="flex items-center justify-center px-6 h-11 rounded-full bg-[#1A1A1A] hover:bg-[#000] transition-colors whitespace-nowrap"
          >
